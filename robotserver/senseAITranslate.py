...
def find_matching_point(left_box, right_box, left_image, right_image):
    ...
    if (mask_success_left is False or mask_success_right is False or return_center_co_only==1):
...
def rotate_gripper(angle):
    command_message={"id": command_id, "command": "rotate_gripper", "angle": angle}
    send_to_redis(command_message)

def approach_gripper(value):
    command_message={"id": command_id, "command": "approach_gripper", "value": value}
    send_to_redis(command_message)


def send_to_redis(command_message):

    # Send the command to Redis
    r.rpush('commands_queue', json.dumps(command_message))
    print(f"Command sent to Redis: {command_message}")
