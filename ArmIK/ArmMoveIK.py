...
# The robotic arm moves according to the angles calculated by inverse kinematics
ik = IK('arm')
...
class ArmIK:
    ...
    def transformAngelAdaptArm(self, theta3, theta4, theta5, theta6):
        # Convert the angles calculated by inverse kinematics to the pulse width values for the servos
    ...
    def setPitchRange(self, coordinate_data, alpha1, alpha2, da=1):
        # Given coordinates and pitch angle range alpha1, alpha2,
        # automatically find a suitable solution within the range
        # Return False if no solution, otherwise return corresponding servo angles and pitch angle
        # Coordinates in cm, passed in as a tuple, e.g., (0, 5, 10)
        # da is the increment for the pitch angle during traversal
    ...
    def setPitchRangeMoving(self, coordinate_data, alpha, alpha1, alpha2, movetime=None):
        # Given coordinates and pitch angle alpha, and the pitch angle range alpha1, alpha2,
        # automatically find the solution closest to the given pitch angle, and move to the target position
        # Return False if no solution, otherwise return servo angles, pitch angle, and run time
        # Coordinates in cm, passed in as a tuple, e.g., (0, 5, 10)
        # alpha is the given pitch angle
        # alpha1 and alpha2 are the pitch angle ranges
        # movetime is the servo rotation time in ms, if not provided, it will be calculated automatically
        x, y, z = coordinate_data
    ...
    def drawMoveRange2D(self, x_min, x_max, dx, y_min, y_max, dy, z, a_min, a_max, da):
        # Test reachable points, display as a 2D plot, with z fixed
        try:
    ...
    def drawMoveRange3D(self, x_min, x_max, dx, y_min, y_max, dy, z_min, z_max, dz, a_min, a_max, da):
        # Test reachable points, display as a 3D plot. If there are too many points, the 3D plot will be difficult to rotate
        fig = plt.figure()
